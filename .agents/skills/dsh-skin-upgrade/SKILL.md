...
用户明确指令优先于本技能的一般流程；先结合会话中已有授权与当前仓库 AGENTS.md 判断适用步骤。已授权步骤直接完成；若技能导致暂停或偏离任务，链接具体文件、引用具体规则并解释适用原因。
...
- 用户提供或明确要求复用社区插件升级卡、通用宿主审计或外部迁移工具时，再读取
  [references/community-plugin-corridor.md](references/community-plugin-corridor.md)；外部结论不能直接替代官方对象复核或当前皮肤矩阵。
- 用户明确要求本技能的效果评测或回归评测时读取
  [references/skill-evaluation.md](references/skill-evaluation.md)；普通升级不因此自动运行 benchmark。
...
3. 优先只读现有官方对象；需要切换版本、安装依赖或避免与用户修改冲突时再创建独立 clone 或 detached worktree。当前工作树可安全编辑时直接修改目标皮肤，不为只读比较强制复制仓库。不要切换正在使用的源码 checkout，也不要把测试包 link 到用户当前 profile，除非用户明确要求运行验证。
...
7. 缺少必要版本或支持范围时，只暂停依赖该证据的适配，继续处理已闭合部分；未获准真实运行或缺运行产物证据时，可完成源码适配与冷检查，但不能宣称已通过实机验收。
...
卡库只存在于接入了脚手架的环境：皮肤仓库里的 `.agents/knowledge/` 是指向脚手架的本机链接，不随皮肤仓库分发；`pnpm host-deltas:*` 定义在脚手架仓库，须在脚手架目录运行。找不到 `.agents/knowledge/dsh-host-deltas/` 或脚手架时，跳过下文第 3、4 步和卡片沉淀，直接从官方对象现场审计，把本应沉淀的宿主事实写进 Skin Adoption Matrix，并在报告里注明未复用、未沉淀卡片。
...
- 目标单版本适配可以替换已失效的旧关系；多版本走廊才保留经过精确源码证明的旧路径。已有相关测试使用精确目标结构，避免旧 fixture 通过掩盖漏项；新增测试或夹具仅在用户要求或已接受的交付包含时进行。没有对应夹具时报告证据缺口，不机械重写无关测试，也不为猜测的 DOM 增加宽泛路径。
...
- 源码或素材变化后运行仓库既有 build，提交型 `lib` 必须同步；目标项目若生成 `skin.build.json`，也须用其既有生成器更新，不得手改 fingerprint。
- 每完成一批编辑立即检查 `git diff --stat` 和关键文件行数。若升级补丁突然删除大段既有定制，先停止并恢复本批由自己产生的改动，再重新以最小 hunk 实施；不要用后续编辑掩盖异常覆盖。

## 验证与报告

按改动风险选择受影响包的必要 build、已有定向测试或 typecheck/lint，并检查 `git diff --check`；不是把所有存在的命令各跑一遍。必要检查通过后，仅在新改动、失败或未解决疑点出现时重跑或扩大。检查最终 bundle 的模块请求、机器绝对路径、远程运行时资产和 source map。不要为了升级默认运行浏览器、GUI、真实 profile 或 E2E。
...
完整证据保存在本轮矩阵；最终回复按本次变化选用以下相关项，引用矩阵而不重复全文，无变化或不适用项可省略：
...
- 实际运行的检查及结果；
- 未运行的真实宿主、视觉、交互或性能验证；
- 一个按影响排序的人机协同清单，只保留需要用户作设计取舍、提供运行证据或授权专项验证/修复的未闭环事项；没有未闭环事项时无需单独列空清单。
