...
对本轮受影响的展示面逐项建立下表，不能用默认状态代表整项通过。先从目标宿主的调用者和条件渲染枚举可达表面，再沿皮肤 CSS 检查颜色与背景来源；包括展开后才出现的左右侧栏、抽屉、二级菜单、阅读面板及其普通 div 外壳，不能只搜索已有 role 或皮肤选择器。

| 表面 / 入口 | 主题与可达状态 | 前景 / 背景 owner 和级联证据 | 静态结论 | 视觉证据或待验收操作 |
|---|---|---|---|---|

- 按组件实际支持的状态检查 normal、hover、focus-visible、active、selected/checked、disabled、busy/error；不存在的状态写不适用。亮暗主题分别检查，并覆盖该组件支持的展开/收起、停靠/浮动/全屏状态。
- 文本和图标沿最终胜出的 color、-webkit-text-fill-color、text-shadow、opacity 及继承 token 检查；尤其核对祖先的宽泛后代规则和伪类覆盖。默认文字正确不证明 hover 或选中项正确。
- 从实际承载文字的表面向后追踪 background、透明渐变、伪元素及祖先底色。透明 token 在页面场景上可用，不代表展开的阅读区也可用；选择器命中、变量存在、背景声明存在均不等于可读性通过。
- 静态发现冲突时记录具体入口、状态、两端样式及最小修复 owner；依赖背景图、透明叠加或实时布局才能判断的项目标未证实，并给用户可操作的复核步骤。没有 GUI 授权时完成静态审计并报告缺口，不自动启动浏览器，也不将缺口写成已适配。获准视觉验证后逐项观察这些状态，记录实际运行产物坐标。
...
结构测试存在时，其 fixture 应对应实际支持的精确版本；多版本走廊分别检查相应结构，目标单版本适配检查目标结构。新增测试遵守用户授权与主技能的验证边界；缺少目标 fixture 时报告覆盖缺口，不能用旧 fixture 通过证明新关系已验证。不机械改写无关测试，不用“兼容”暗示旧版仍被验证；manifest 的最后验证 RC 不自动构成多版本支持承诺。
