		...
		*/
		function apply(ctx) {
			...
			const syncProjectedState = () => {
				...
				set(PROJECTED_STATE_ATTRIBUTES.settingsInSidebar, document.querySelector(SETTINGS_SLOT_DIALOG_SELECTOR) !== null);
			};
			...
			const syncSettingsBackdropFrame = () => {
				...
				const mask = document.querySelector(SETTINGS_SLOT_DIALOG_SELECTOR)?.parentElement?.querySelector(SETTINGS_MASK_SELECTOR) ?? null;
			...
			observer = new MutationObserver((records) => {
				...
				for (const record of records) {
					...
					if (!settingsStateChanged && appNodes.length > 0 && target !== void 0 && (target.closest(SETTINGS_OWNER_SELECTOR) !== null || target === body && appNodes.some((node) => node.querySelector(`:scope > ${SETTINGS_PORTAL_DIALOG_SELECTOR}`) !== null))) settingsStateChanged = true;
