...
- 亮/暗各一对空态/工作态 16:9 场景（亮色白色机械舱、暗色夜间书房），640ms 交叉淡化，素材随包分发并由皮肤同源路由加载
...
```sh
dsh plugin --profile web add '@smalltailqwq/dsh-client-ui-skin-deep-whale-manager' && dsh plugin --profile web add '@smalltailqwq/dsh-client-ui-skin-orca-link'
```

PowerShell 版本见仓库 README。未指定 dist-tag 时 npm 使用 `latest`。首次安装后重启一次 DSH，然后在“设置 → 皮肤管理”中选择“虎鲸链路”；之后切换走配置热重载。独立子包 link 仅用于本地开发。
...
构建预设源自 [zhu1090093659/dsh-web-ui](https://github.com/zhu1090093659/dsh-web-ui)，当前皮肤在本仓库维护。在本皮肤目录执行：

```sh
npm run build       # 校验静态图片、构建两端 JS、生成指纹
npm test            # 行为测试
```

PNG/WebP 以原字节随包分发，通过宿主同源资源路由加载。升级时需同步 `lib/`、`assets/runtime/` 与 `skin.build.json`；仅替换客户端文件会缺少图片服务。资源路径、缓存与素材更新方式见 [静态图片说明](../docs/asset-delivery.md)。
