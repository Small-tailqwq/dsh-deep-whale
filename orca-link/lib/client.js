	...
	factory: (require) => {
		...
		*/
		function installOrcaComposerCollapse(body) {
			...
			const positionRestore = (binding, sourceRect) => {
				...
				const top = clamp(rootRect.top + rootRect.height * anchor.topRatio, rootRect.top + 8, rootRect.bottom - RESTORE_SIZE - 8);
			...
			const mountBinding = (seat) => {
				...
				if (!composerBelongsToConversation$1(root)) {
		...
		*/
		...
		function installOrcaSettingsOverlay(body) {
			...
			const observer = new MutationObserver((records) => {
				if (hasMutationOutsideTerminal(records)) synchronize(records.some((record) => record.type === "childList" && record.target instanceof Element && record.target.closest("[data-slot='sidebar.settings']") !== null));
				synchronizeTheme();
			});
			const dispose = () => {
				observer.disconnect();
