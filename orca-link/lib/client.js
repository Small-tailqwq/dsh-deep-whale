	...
	factory: (require) => {
		...
		*/
		...
		function installOrcaSettingsOverlay(body) {
			...
			const observer = new MutationObserver((records) => {
				if (hasMutationOutsideTerminal(records)) synchronize(records.some((record) => record.type === "childList" && record.target instanceof Element && record.target.closest(SETTINGS_OWNER_SELECTOR) !== null));
				synchronizeTheme();
			});
			const dispose = () => {
				observer.disconnect();
