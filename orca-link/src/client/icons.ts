 ...
 * assistant messages, but the conversation flow marks every generic tool row
 * with it, and those rows become the wrench the host itself draws for tools.
 */
function contextualName(svg: SVGElement, name: string): string {
  if (name === 'sparkle') {
    return svg.closest('[data-chat-flow-kind="tool"]') !== null ? 'wrench' : name
  }
