...
describe('Orca Link skin apply', () => {
  ...
  it('redraws the generic tool row as a diagonal wrench and leaves other sparkles alone', async () => {
    // The conversation flow marks every generic tool row with the sparkle glyph,
    // while the trajectory view uses that same glyph for assistant messages —
    // and the host itself draws a wrench for tool kinds.
    document.body.innerHTML = `
      <div data-chat-flow-kind="tool">
